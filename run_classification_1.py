...
def add_knowledge_worker(params):
    ...
    for line_id, line in enumerate(sentences):
        ...
        try:
            ...
            elif len(line) == 6:  # for dbqa
                label = int(line[columns["label"]])
                text_a = line[columns["text_a"]]
                op_a = line[columns["opa"]]
                op_b = line[columns["opb"]]
                op_c = line[columns["opc"]]
                op_d = line[columns["opd"]]
                text = CLS_TOKEN + text_a + SEP_TOKEN + text_a + ' '+ op_a + SEP_TOKEN + text_a + ' '+ op_b + SEP_TOKEN + text_a + ' '+ op_c + SEP_TOKEN + text_a + ' '+ op_d + SEP_TOKEN
    ...
    # print("In add dataset count:", count) 
    print("In add dataset:", len(dataset)) 
    return dataset
...
def main():
    ...
    if not args.testing:
        ...
        for epoch in range(1, args.epochs_num+1):
            ...
            for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch) in enumerate(batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms)):
                ...
                if (i + 1) % args.report_steps == 0:
                    ...
                    sys.stdout.flush()
                    total_loss = 0.
                total_steps += 1  # 增加总的训练步数
                loss.backward()
                optimizer.step()
            # 在每个训练周期结束后，将该周期的平均损失添加到列表中
            losses_per_epoch.append(total_loss / (total_steps // args.report_steps))  # 计算平均损失


            print("Start evaluation on dev dataset.")
            result = evaluate(args, False)
            dev_accuracies.append(result)
        ...
        # 在第一个子图上绘制损失的收敛图
        ax1.plot(range(1, len(losses_per_epoch) + 1), losses_per_epoch, label='Training Loss')
        ...
        # 在第二个子图上绘制dev集准确度曲线
        ax2.plot(range(1, len(dev_accuracies) + 1), dev_accuracies, label='Dev Accuracy', color='orange')
        ax2.set_title('Dev Set Accuracy')
        ...
        # 保存为图片文件
        plt.savefig('./outputs/new/with_umls_pub.png')
